...
export function markdownVscodeContributions({
  ...
  const inputFile = fs.readFileSync(path.join(appRoot.path, inputPath), "utf8").replace(/\r?\n/g, EOL);
  const packageFile = fs.readFileSync(path.join(appRoot.path, packagePath), "utf8");
  ...
  if (fs.existsSync(path.join(appRoot.path, outputPath))) {
    const outputInitialText = fs.readFileSync(path.join(appRoot.path, outputPath), "utf8");
