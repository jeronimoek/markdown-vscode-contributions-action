...
import { unicodePadEnd } from "./utils.mjs";
const {
  EOL
} = require("os");
export function tableToMarkdown(table) {
  ...
  for (let row = 0; row < rows; row++) {
    ...
    for (const columnProps of Object.values(table.columns)) {
      tableString += " ";
      if (row === 0) {
        tableString += unicodePadEnd(columnProps.columnHeader, columnProps.width);
      } else if (row === 1) {
        tableString += unicodePadEnd("", columnProps.width, "-");
      } else {
        tableString += unicodePadEnd(columnProps.values[row - 2] || "-", columnProps.width);
      }
      tableString += " |";
    }
    tableString += EOL;
