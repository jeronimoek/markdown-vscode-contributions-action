...
function markdownVscodeContributions({
  ...
  const inputFile = _fsExtra.default.readFileSync(_path.default.join(_appRootPath.default.path, inputPath), "utf8").replace(/\r?\n/g, _os.EOL);
  const packageFile = _fsExtra.default.readFileSync(_path.default.join(_appRootPath.default.path, packagePath), "utf8");
  let outputText = inputFile;
  const tables = (0, _getTablesWithData.getTablesWithData)(packageFile, inputFile);
